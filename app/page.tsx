...
export default function Home() {
  const content = pageContentConfig.beranda;
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <section className="relative px-4 sm:px-6 lg:px-8 py-20 md:py-32 overflow-hidden bg-gray-50 flex items-center justify-center">
        ...
        <div className="absolute inset-0 opacity-5 pointer-events-none" style={{ backgroundImage: "url('data:image/svg+xml,%3Csvg width=\\'60\\' height=\\'60\\' viewBox=\\'0 0 60 60\\' xmlns=\\'http://www.w3.org/2000/svg\\'%3E%3Cg fill=\\'none\\' fill-rule=\\'evenodd\\'%3E%3Cg fill=\\'%232563eb\\' fill-opacity=\\'1\\'%3E%3Cpath d=\\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')" }}></div>
        
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10 w-full">
          <div className="flex flex-col items-start text-left">
            <span className="inline-block py-1 px-3 rounded-full bg-blue/10 text-blue-dark font-medium text-sm mb-6 border border-blue/20">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/kontak" className="px-6 py-3.5 bg-blue text-white font-semibold rounded-full hover:bg-blue-dark transition-all duration-300 shadow-lg shadow-blue/30 flex items-center">
                Jadwalkan Konsultasi <ArrowRight size={18} className="ml-2" />
              </Link>
              <Link href="/program" className="px-6 py-3.5 bg-white text-darker border border-gray-200 font-semibold rounded-full hover:border-blue hover:text-blue transition-all duration-300">
                Lihat Program
              </Link>
            </div>
          </div>
      ...
      {/* STATS SECTION */}
      <section className="bg-blue-dark py-12 relative z-20 -mt-8 mx-4 sm:mx-6 lg:mx-auto lg:w-full lg:max-w-6xl rounded-2xl shadow-xl overflow-hidden text-center md:text-left">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 px-8">
          {statsData.map((stat, idx) => (
            <div key={idx} className="flex flex-col items-center md:items-start text-center md:text-left">
              ...
              <span className="text-blue-light text-sm font-medium uppercase tracking-wider">{stat.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
          ...
          <div className="md:col-span-7 order-1 md:order-2">
            <h2 className="text-3xl md:text-4xl font-display font-bold text-darker mb-6">{content.aboutTitle}</h2>
            ...
            <ul className="space-y-3 mb-8">
              {["Pendekatan berbasis riset (Research-based)", "Spesialisasi integrasi AI dalam kurikulum", "Penggerak ekosistem pesantren digital", "Trainer bersertifikat internasional"].map((item, i) => (
                <li key={i} className="flex items-center text-gray-700">
                  <CheckCircle2 size={20} className="text-blue mr-3 shrink-0" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
             <Link href="/tentang" className="inline-flex items-center text-blue font-semibold hover:text-blue-dark transition-colors">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {servicesData.map((service) => {
              ...
              return (
                <div key={service.id} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-blue/20 transition-all duration-300 group">
                  <div className="w-14 h-14 bg-blue/10 text-blue rounded-xl flex items-center justify-center mb-6 group-hover:bg-blue group-hover:text-white transition-colors">
                    <Icon size={28} />
                  </div>
                  ...
                  <Link href={`/layanan#${service.id}`} className="text-blue text-sm font-semibold inline-flex items-center group-hover:underline">
